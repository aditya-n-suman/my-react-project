...
function initDB() {
  ...
  db.serialize(() => {
    ...
    db.run("CREATE INDEX IF NOT EXISTS idx_var_name ON variable_refs(variable_name)");
    db.run("CREATE INDEX IF NOT EXISTS idx_file_path ON variable_refs(file_path)");
  });
...
async function addFile(db, filePath) {
  ...
  db.serialize(() => {
    ...
    try {
      ...
      refs.forEach(ref => {
        stmt.run(ref.name, ref.filePath, ref.line, ref.type || 'declaration', ref.source || null);
      });
    ...
    db.all(sql, [], (err, rows) => {
      ...
      const scored = rows.map(r => ({
        ...
        variables: JSON.parse(r.vars).reduce((acc, v) => {
          ...
          if (!v.name || typeof Object.prototype[v.name] !== 'undefined') return acc;
...
async function askQwen(query, refreshContext = false) {
  ...
  const contextText = results.map(r => `${r.path}:\n${r.content}`).join("\n\n");

  // Setup fresh context if needed
  if (!currentContext || refreshContext) {
    log.info(refreshContext ? 'Refreshing context...' : 'Initializing new context...');
...
export { initDB, addFile, search, askQwen, cleanup, DB_PATH };
